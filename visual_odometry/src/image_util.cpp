#include "../include/visual_odometry/image_util.h"
...
void ImageUtil::saveKeypointsImage(const std::string file_name)
{
  ...
  //cv::imwrite(ament_index_cpp::get_package_prefix("visual_odometry") + "/figures/" + file_name + ".png", img_keypoints);
}
    ...
    const cv::Mat& image0, const cv::Mat& image1, const std::vector<cv::KeyPoint>& keypoints0)
{
  ...
  // RCLCPP_INFO(rclcpp::get_logger("image_util_logger"),"Optical Flow: total candidates = %ld",
